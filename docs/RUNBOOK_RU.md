...
- `scripts/package_portable_linux.sh` - сборка Linux core portable-папки `dist/NetStitch-linux64-portable` с `NetStitch`, `NetStitch.desktop`, app icon, native `.so` runtime libraries в `libs/`, `apps/`, `language/`, `storage/`; внешние integration-модули не включаются по умолчанию и добавляются только явным `--package-modules <id>` или `NETSTITCH__PACKAGE_MODULES` (`*` явно включает все repo-local modules), при этом их `locales/` копируются внутрь папки соответствующего модуля; при packaging удаляются только известные устаревшие SDK/test integration folders `hello-world-rust`, `hello-world-cpp`, `ui-entity-showcase`; запускается из Linux/WSL, использует отдельный `target/linux-portable`, чтобы не смешивать Windows и Linux cargo-кэш, наследует общий package revision из `NETSTITCH__PACKAGE_REVISION` или Windows portable manifest и подходит для WSLg GUI-smoke перед Linux release; packaging ставит `storage/clear-system-events-on-next-start`, чтобы следующий запуск очищал только старую историю `system_events`; included `install-desktop-launcher.sh` умеет поставить user-local launcher из текущей portable-папки или скопировать portable в `~/.local/opt/netstitch` через `--copy-to`, а также выводит подсказки по недостающим GTK/WebKit/runtime libraries для Debian/Ubuntu, Debian 13, Fedora, Arch и openSUSE
- `scripts/package_linux_deb.sh` - сборка installable Linux `.deb` из той же Linux portable staging-папки; пакет кладёт read-only runtime в `/opt/netstitch`, wrapper `/usr/bin/netstitch`, launcher `/usr/share/applications/netstitch.desktop`, иконку приложения из `resources/shin0by.png`, объявляет GTK/WebKit/AppIndicator/xdo/OpenSSL/SQLite runtime-зависимости и команды диагностики `iputils-ping`/`traceroute`/`dnsutils`, затем пишет артефакт `dist/release-assets/netstitch_<version>_amd64.deb`; wrapper перед стартом создаёт `~/.local/share/netstitch`, копирует туда shipped `apps/` и `integrations/` без перетирания пользовательских файлов, выставляет `NETSTITCH__DATA_DIR`, `NETSTITCH__CONNECTORS_DIR`, `NETSTITCH__INTEGRATIONS_DIR`, поэтому SQLite, редактируемые connectors и module data пишутся в пользовательский каталог; для тестирования Linux установочного контура ставить именно этот пакет через package manager, а не запускать portable напрямую
- `scripts/package_module_sdk_examples.ps1` - сборка и упаковка двух SDK-примеров `ui-entity-showcase-rust` и `ui-entity-showcase-cpp` в ready-to-install архивы `docs/module-sdk/packages/*-{windows,linux}-x86_64.zip`; Windows библиотеки собираются локально, Linux библиотеки собираются только через отдельный тестовый WSL `NetStitch-Linux-Test`; после любого изменения SDK-примеров или их `module.json` архивы нужно пересобрать этим скриптом и закоммитить вместе с исходниками
- `scripts/install_module_sdk_examples_to_portable.ps1` - установка свежих Windows-архивов showcase-модулей из `docs/module-sdk/packages/` в локальный `dist\NetStitch-win64-portable\integrations` для ручной UI-проверки; скрипт сохраняет module-owned `data\`, перезаписывает manifest/locales/bin/source из архива и останавливается, если NetStitch запущен из этой portable-папки
...
- `scripts/package_release_assets.ps1` - сборка Windows portable-папки и zip release asset `NetStitch-win64-portable-<version>.zip` в `dist\release-assets`; в корне zip лежит папка `NetStitch-win64-portable\`; staging-копия очищается от runtime `storage` и `integrations\<module>\data`, но сохраняет `integrations\<module>\locales`; если `-Version` не передан, имя zip берётся из `modules.app.full_version` portable manifest
- `scripts/package_release_assets_linux.sh` - сборка Linux portable-папки и tar.gz release asset `NetStitch-linux64-portable-<version>.tar.gz` в `dist/release-assets`; в корне архива лежит папка `NetStitch-linux64-portable/`; staging-копия очищается от runtime `storage` и `integrations/<module>/data`, но сохраняет `integrations/<module>/locales`
- `.github/workflows/release.yml` - GitHub portable release workflow: push в ветку `release` запускает workflow `Portable Release`. Он собирает Windows zip, Linux tar.gz и Linux `.deb` на GitHub runners, берёт базовую версию из `[workspace.package].version`, а full-version/tag - из tracked `config\release-version.json`, который синхронизируется с локально протестированной portable-сборкой. Если release-version file отсутствует, workflow падает обратно на tags fallback. Затем workflow передаёт revision в оба portable package script-а и после успешной сборки создаёт tag, GitHub Release и public assets. Manual workflow dispatch остаётся только для явно подтверждённого повтора/аварийного запуска. Generated `dist\...\config\version-manifest.json` хранит версию уже собранной portable-папки и не коммитится; tracked source-of-truth для чистого GitHub runner - `config\release-version.json`.
...
- Очистка облачной D1 выполняется только явной operator-командой. Для удаления только данных приложений, портов, протоколов, доменов и observation-публикаций использовать `scripts\clear_cloud_database.ps1 -Scope AppData -Yes`: скрипт удаляет `app_catalog`, `observations`, `observation_author_rows`, `observation_submissions`, `domain_verifications`, но сохраняет users, clients, sessions, keys, authors, OAuth/captcha/rate-limit/browser-verification states, quota windows, audit rows, schema/migrations и служебные Cloudflare/D1 metadata. Полный reset остаётся отдельной разрушительной операцией `scripts\clear_cloud_database.ps1 -Scope All -Yes` (или старый совместимый `-Yes`) и удаляет весь клиентский след: пользователей, идентификаторы клиентов, сессии, ключи, авторов, каталог приложений, наблюдения, OAuth/captcha/rate-limit/browser-verification состояния, quota windows, audit/cache rows.
...
- Overlay модуля показывает module-provided status без badge-статусов, detected/export paths как read-only input-like поля и progress-блок после старта module download; все длинные пути отображаются через общий однострочный path-field с горизонтальной прокруткой. Открытие overlay и ввод пути не запускают тяжёлую проверку файловой системы; root validation выполняется только при явном module action. Для долгих blocking `ui_action` progress/status должны обновляться через ABI event `IntegrationHostEvent.event = "ui_values"` и endpoint `/v1/integrations/ui-action-events`; desktop/browser UI опрашивают эти события по `ui_action_token`, а `download_progress` не является module UI alias-ом.
- Overlay модуля использует стандартный host-owned footer: `footer`-сущности схемы выводятся в левом slot-е, а кнопка `Назад` добавляется host-ом справа по умолчанию. Если кнопка отсутствует, сначала проверить активную `footer`-сущность и флаг `hide_host_back_button`, затем parity desktop/browser renderer-ов; отдельные body/footer-кнопки назад в схемах модуля не являются штатным layout-контрактом.
...
- перед итоговым ответом после любой завершённой работы Codex нужно выполнить `scripts/compile.ps1`, чтобы пользователь всегда мог тестировать актуальную portable-выкладку в `dist\NetStitch-win64-portable`; если менялись SDK showcase-модули, после `scripts/package_module_sdk_examples.ps1` дополнительно выполнить `scripts/install_module_sdk_examples_to_portable.ps1`, потому что core compile не включает внешние SDK-примеры по умолчанию

Стандартный локальный набор действий при изменении версии проекта или сборочной ревизии:
1. Собрать Windows portable через `scripts\compile.ps1` и проверить `dist\NetStitch-win64-portable\config\version-manifest.json`: `app`, `watcher` и `tool` должны иметь один full-version.
2. Собрать Linux portable и installable пакет только в отдельном WSL `NetStitch-Linux-Test`, не в Debian WSL: `wsl -d NetStitch-Linux-Test -- bash -lc 'cd /mnt/c/Programming/GitHub/Netstitch && bash scripts/package_linux_deb.sh --version <full-version>'`.
3. Установить свежий пакет именно через package manager тестового Linux: `wsl -d NetStitch-Linux-Test --user root -- apt-get install -y /mnt/c/Programming/GitHub/Netstitch/dist/release-assets/netstitch_<full-version>_amd64.deb`; не считать простой запуск `dist/NetStitch-linux64-portable/NetStitch` проверкой install flow.
4. Проверить установленную Linux-версию: `dpkg -s netstitch`, `/opt/netstitch/config/version-manifest.json`, `/usr/bin/netstitch`, `/usr/share/applications/netstitch.desktop`, `/home/netstitch/Desktop/NetStitch.desktop`, `ldd /opt/netstitch/NetStitch` без `not found`.
5. Проверить shipped app-коннекторы: в `/opt/netstitch/apps` и user-local `~/.local/share/netstitch/apps` должны быть `.app` файлы; runtime не должен создавать их сам при запуске, только читать поставленные packaging/install flow файлы.
6. Проверить Linux-модули без дублей: активный runtime должен читать один user-local набор из `~/.local/share/netstitch/integrations` через wrapper `NETSTITCH__INTEGRATIONS_DIR`; если после ручных тестов остались копии одних и тех же showcase-модулей в нескольких местах, удалить лишнюю копию и оставить один актуальный набор. `/opt/netstitch/integrations` не должен быть вторым активным источником для установленного пакета.
7. Если обновлялись Module SDK examples, для Windows UI smoke пересобрать `scripts\package_module_sdk_examples.ps1`, затем выполнить `scripts\install_module_sdk_examples_to_portable.ps1`; перед Linux smoke пересобрать те же архивы и обновить installed showcase-модули из `docs/module-sdk/packages/*-linux-x86_64.zip` в `~/.local/share/netstitch/integrations`.
8. Только после локального теста пользователя и отдельной команды выполнять commit/push/release; без прямой команды коммиты и push не делать.
...
3. Если менялся пользовательский runtime или packaging, выполнить `scripts\compile.ps1`; для Linux portable packaging отдельно проверить `scripts/package_portable_linux.sh` из Linux/WSL контура, а для установочного Linux smoke собрать и поставить `scripts/package_linux_deb.sh`.
...
8. Дождаться GitHub Actions `Portable Release`: он должен собрать `NetStitch-win64-portable-<full-version>.zip`, `NetStitch-linux64-portable-<full-version>.tar.gz` и `netstitch_<full-version>_amd64.deb`, проверить наличие Windows `WinDivert.dll`/`WinDivert64.sys`, Windows native DLL под `libs\`, Linux native `.so` под `libs/`, Linux `/usr/bin/netstitch` и desktop launcher в `.deb`, создать tag `v<full-version>`, опубликовать GitHub Release и прикрепить архивы/пакет.
...
Сетевая диагностика строки `Monitoring`:
- нажать `...` перед delete-`X`; панель `Monitoring` сменится на `Сетевая диагностика`
- для `Ping`/`Трассировка` проверить IP строки или изменить target; для `DNS lookup` проверить домен/IP и при необходимости указать DNS-сервер IP-адресом
- пустой DNS-сервер означает resolver, настроенный в ОС; DNS-сервер не применяется к ping/trace
- операция выполняется в фоне, максимум 50 секунд после получения native tool lock; общий watcher deadline с ожиданием lock - 120 секунд
- stdout/stderr должны появляться в поле результата построчно во время выполнения; desktop и browser опрашивают только `/v1/network-diagnostics/{job_id}`, не обновляя общий snapshot
- `Назад` отменяет только ожидание результата в UI и возвращает таблицу; завершившийся устаревший результат не применяется к закрытой или заново открытой панели
- Windows использует системные `ping.exe`, `tracert.exe`, `nslookup.exe`; installable Linux `.deb` подтягивает `iputils-ping`, `traceroute`, `dnsutils`
- для Linux portable без package manager `install-desktop-launcher.sh` сообщает отсутствующие команды и подсказывает имена пакетов для Debian/Ubuntu, Fedora, Arch и openSUSE
...
- для диагностики runtime-коннекторов запустить discover с `NETSTITCH__CONNECTORS_DIR=resources\connectors\apps` и проверить, что `.app` manifests дают ожидаемый список коннекторов; для community-манифестов отдельно проверить применимые правила `root_paths` и `registry_strings`, если приложение ищется через корни дисков, папки Games или строковые значения реестра
