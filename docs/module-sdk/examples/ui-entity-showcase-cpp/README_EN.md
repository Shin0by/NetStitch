...
- `simulate_download` demonstrates an explicit `26px` button height shared by desktop/browser, sends live `IntegrationHostEvent.event = "ui_values"` events while the blocking `ui_action` is still running, and animates the large `showcase-progress` bar from `0` to `100` through `Queued`, `Processing`, and `Done`; buttons without explicit height keep automatic sizing;
