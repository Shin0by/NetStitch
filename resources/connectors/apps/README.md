...
- `kind = "known_path"` / путь относительно env-переменной; поля `root_env`, `relative_path`; в сегментах разрешён `*`, например `app-*`.
...
Если `[[discovery]]` не указан, NetStitch попробует использовать `[[discovery_sources]]` как fallback для `app_paths`, `fixed_path` и `known_path` с прямым путём.
